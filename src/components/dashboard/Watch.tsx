  ...
  return (
    <WatchRoot>
      ...
      <WatchBody>
        ...
        <Reflaction />
        <TimeDisplay>
          <DateRow time={time} colors={colors} />
...
const WatchRoot = ({ children }: { children: ReactNode }) => {
  return (
    <motion.div
      ...
      className="relative w-40 h-45 select-none "
    >
      {children}
    </motion.div>
  );
...
const WatchBody = ({ children }: { children: ReactNode }) => {
  return (
    <motion.div className="relative bg-gray-900/70 rounded-xl p-4 w-full h-full shadow-xl border border-gray-700/50 overflow-hidden backdrop-blur-sm">
      {children}
    </motion.div>
  );
};
...
const MainTime = ({ time, colors }: { time: Time; colors: Colors }) => {
  return (
    <div className="text-center space-y-1">
      <div className="font-mono text-4xl font-light tracking-tighter text-white drop-shadow-[0_0_6px_rgba(0,0,0,0.8)]">
        {time.hours}:
        <span
      ...
      <div className="flex justify-center items-center gap-1">
        <span className="font-mono text-xs text-gray-300 drop-shadow-[0_0_4px_rgba(0,0,0,0.8)]">
          {time.seconds}
        </span>
...
const DateRow = ({ time, colors }: { time: Time; colors: Colors }) => {
  return (
...
const ProgressBar = ({ time, colors }: { time: Time; colors: Colors }) => {
  return (
    <div className="space-y-1.5">
      <motion.div
        animate={{ width: `${(Number(time.minutes) / 60) * 100}%` }}
        transition={{ duration: 0.5 }}
      ...
      <motion.div
        ...
        transition={{ duration: 0.5 }}
        className="h-0.5 rounded-full shadow-lg"
        style={{
          backgroundColor: colors.primary,
          opacity: 0.4,
...
const Background = ({ colors }: { colors: Colors }) => {
  return (
    <div
      className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${colors.bgGradient}`}
    >
